...
public class CustomTextView extends TextView {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        logger = new RouteLog();
        logger.setLoggerClass(CustomTextView.class);
    ...
    private void setCustomFont(Context ctx, AttributeSet attrs) {
        ...
        autoResize = a.getBoolean(R.styleable.FontText_autoResize, false);
...
//    }
    ...
    protected void onDraw(Canvas canvas) {
        if (justify) {
            ...
            for (int i = 0; i < layout.getLineCount(); ++i) {
                ...
                String str = "<sup><small>®</small></sup>";
                line = line.replaceAll("®", str);
                Spanned finalLine = Html.fromHtml(line);
                float width = StaticLayout.getDesiredWidth(text, lineStart, lineEnd, this.getPaint());
                if (this.needScale(finalLine)) {
                    this.drawScaledText(canvas, lineStart, finalLine, width);
                } else {
                    canvas.drawText(finalLine.toString(), 0.0F, (float) this.mLineY, paint);
                }
    ...
    private void drawScaledText(Canvas canvas, int lineStart, Spanned line, float lineWidth) {
        float x = 0.0F;
...
//            line = line.substring(3);
    ...
    private boolean isFirstLineOfParagraph(int lineStart, Spanned line) {
        return line.length() > 3 && line.charAt(0) == 32 && line.charAt(1) == 32;
    }

    private boolean needScale(Spanned line) {
        return line.length() != 0 && line.charAt(line.length() - 1) != 10;
    }
